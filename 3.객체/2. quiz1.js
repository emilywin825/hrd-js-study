...
while(true){
    var inputId=prompt('아이디를 입력해주세요.');
    var findUser=null;
    for(var arr of userInfo.userList){
        if(inputId===arr.account){
            findUser=arr; 
            break;
        }
    }
    ...
    if(findUser!==null){
        var inputPw=alert(`존재하는 아이디입니다.`);

        while(true){
                var inputPw=prompt(`비밀번호를 입력해주세요.`);
                if(inputPw===findUser.password){
                    alert(`${findUser.username}님 반갑습니다.\n로그인에 성공하셨습니다.`);
                break;
            }else if(inputPw!==findUser.password){
