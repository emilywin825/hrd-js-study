...
while(true){
    var memberIsHere = false;
    var deleteMem = prompt(`현재 멤버 : [${member}]\n수정할 이름을 입력하세요`);
    if(member.includes(deleteMem)){
        var deleteMemIndex=member.indexOf(deleteMem);
         var newMem = prompt(`새로운 멤버 이름을 입력하세요.`);
         member.splice(deleteMemIndex,1,newMem);
         alert(`수정 완료!\n현재 멤버 : [${member}]`);
         break;
    }else{
        alert(`${newMem}은(는) 잘못된 이름입니다.\n다시 입력하세요!`)
    }
}
